...
class CrowdDetectionGRPCClient(ObjectDetectionGRPCClient):
    def __init__(
        ...
        super().__init__(model_name='crowd_detection', encoding_quality=encoding_quality,
    ...
    def monolythic_preprocess(
        ...
        image = cv2.copyMakeBorder(image, top, bottom, left, right, cv2.BORDER_CONSTANT, value=(114, 114, 114)).astype(
            'float32') / 255.0
    ...
    def monolythic_postprocess(
        ...
        for output in outputs_from_model:
            if len(output):
                output[:, :4] = self._monolythic_scale_coords((resized_height, resized_width), output[:, :4],
                                                              (original_height, original_width, 3)).round()

                for *xyxy, _, cls_int in reversed(output):
                    c1, c2 = ((xyxy[0] / original_width), (xyxy[1] / original_height)), (
                    (xyxy[2] / original_width), (xyxy[3] / original_height))
