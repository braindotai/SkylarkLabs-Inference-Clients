...
class FaceOrientationDetection:
    ...
    def perform_inference(self, input_batch):
        blob = cv2.dnn.blobFromImage(input_batch[0], scalefactor=1.0, size=(self.img_w_new, self.img_h_new), mean=(0, 0, 0), swapRB=True, crop=False)
    ...
    def decode(self, heatmap, scale, offset, landmark, size, threshold=0.1):
        ...
        c0, c1 = np.where(heatmap > self.inference_params['threshold'])
        if self.landmarks:
            boxes, lms = [], []
        else:
            boxes = []
