...
class ChangeDetectionLocalClient:
    def __init__(
        self,
        threshold: int = 2,
        min_suspect_area: int = 500,
        smoothing_field: int = 21,
    ):
        self.background_subtractor = cv2.bgsegm.createBackgroundSubtractorMOG()
        self.frame_idx = 0
        self.threshold = threshold
        self.min_suspect_area = min_suspect_area
        self.smoothing_field = smoothing_field

    def perform_inference(self, frame, to_plot_on):
        self.frame_idx += 1
        frame = cv2.blur(frame, (self.smoothing_field, self.smoothing_field))
        ...
        ret, current_changes = cv2.threshold(vectors, self.threshold, 255, cv2.THRESH_BINARY)

        # self.accum_image = cv2.add(self.accum_image, current_changes)
        color_image_video = cv2.applyColorMap(current_changes, 2)
        contours, hierarchy = cv2.findContours(current_changes, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)

        for contour in contours:
            x, y, w, h = cv2.boundingRect(contour)

            if w * h >= self.min_suspect_area:
